...
| `has_event_type_concept` | array of URIs | No | Matched on `ado:hasEventTypeConcept`. Each value must be a concept of the Artsdata Event Types vocabulary, `adr:ArtsdataEventTypes` (`http://kg.artsdata.ca/resource/ArtsdataEventTypes`), e.g. `http://kg.artsdata.ca/resource/ClassicalMusicPerformance`. A schema.org class such as `http://schema.org/MusicEvent`, a bare label, or a concept from another publisher's vocabulary in the graph returns nothing rather than an error. Resolve a label with `search_entities` (`types: ["…skos/core#Concept"]`, `in_scheme: ["…/ArtsdataEventTypes"]`), or list the vocabulary with `sparql_query`: `?concept skos:inScheme <http://kg.artsdata.ca/resource/ArtsdataEventTypes> ; skos:prefLabel ?label`. |
